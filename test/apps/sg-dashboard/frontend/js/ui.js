...
export function renderDashboardItems(items, isReadOnly = false, links = []) {
  ...
  if (coreList && stratList) {
    ...
    const coreSkills = skills.filter((s) => {
      ...
      return (
        cat.startsWith("core:") ||
        (!cat.startsWith("strategic:") &&
          !cat.includes("strategic") &&
          !cat.includes("transformation"))
      );
    });
    ...
    document.getElementById("skills-count-pill").textContent =
      `${coreSkills.length} Core • ${stratSkills.length} Strategic`;

    if (coreSkills.length === 0) {
      coreList.innerHTML =
        '<li class="text-gray-500 italic py-1 pl-2">No core skills listed.</li>';
    } else {
    ...
    if (stratSkills.length === 0) {
      stratList.innerHTML =
        '<li class="text-gray-500 italic py-1 pl-2">No strategic skills listed.</li>';
    } else {
  ...
  if (gapsList) {
    ...
    document.getElementById("gaps-count-pill").textContent =
      `${criticalCount} Critical • ${mediumCount} Medium • ${lowCount} Low`;

    if (gaps.length === 0) {
      gapsList.innerHTML =
        '<li class="text-gray-500 italic py-1 pl-2">No skill gaps identified.</li>';
    } else {
      gaps.forEach((item) => {
        // Find plans linked to this gap
        const linkedPlanIds = links
          .filter((l) => l.target_id === item.id)
          .map((l) => l.source_id);
        gapsList.appendChild(
          createGenericElement(item, isReadOnly, linkedPlanIds),
        );
      });
    }
  }
  ...
  if (stratPlansList && tactPlansList) {
    ...
    const overdueCount = plans.filter(
      (p) => p.status !== "completed" && isQuarterPassed(p.target_quarter),
    ).length;
    ...
    if (stratPlans.length === 0) {
      stratPlansList.innerHTML =
        '<li class="text-gray-500 italic py-1 pl-2">No strategic plans scheduled.</li>';
    } else {
      stratPlans.forEach((item) => {
        // Find gaps linked to this plan
        const linkedGapIds = links
          .filter((l) => l.source_id === item.id)
          .map((l) => l.target_id);
        const linkedGapsList = gaps.filter((g) => linkedGapIds.includes(g.id));
        stratPlansList.appendChild(
          createTrainingPlanElement(item, gaps, linkedGapsList, isReadOnly),
        );
      });
    }
    ...
    if (tactPlans.length === 0) {
      tactPlansList.innerHTML =
        '<li class="text-gray-500 italic py-1 pl-2">No tactical plans scheduled.</li>';
    } else {
...
export function getGenericBadgeStyle(category) {
  const base =
    "px-2 py-0.5 rounded text-[9px] font-black tracking-wide uppercase select-none cursor-pointer ";
  let val = category || "Low";
  if (val.includes(":")) {
    val = val.split(":")[1];
  }
  if (val === "Critical")
    return base + "bg-red-500 text-white shadow-sm shadow-red-500/20";
...
function createGenericElement(item, isReadOnly = false, linkedIds = []) {
  ...
  li.className =
    "group flex items-start justify-between py-1 px-2 rounded hover:bg-[var(--bg-hover)] transition-all text-[var(--text-primary)] font-medium border border-transparent";
  ...
  titleSpan.className =
    "font-medium break-words whitespace-normal item-title-text flex-1 min-w-0" +
  ...
  if (item.section === "gap") {
    ...
    countPill.className =
      "count-badge" + (linkedIds.length > 0 ? " active-count" : "");
  ...
  if (!isReadOnly) {
    ...
    deleteBtn.className =
      "text-red-400 hover:text-red-300 opacity-0 group-hover:opacity-100 transition-opacity font-bold px-1.5 py-0.5 rounded text-[10px]";
    deleteBtn.innerHTML = "✕";
    deleteBtn.onclick = (e) => {
      e.stopPropagation();
      deleteItem(item.id);
...
function createTrainingPlanElement(
  item,
  allGaps,
  linkedGaps,
  isReadOnly = false,
  ...
  li.id = `item-row-${item.id}`;
  li.className =
    "group flex flex-col gap-2 p-2.5 rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] hover:bg-[var(--bg-hover)] hover:border-[var(--accent)] transition-all text-[var(--text-primary)] font-medium relative w-full";
  ...
  const isOverdue =
    item.status !== "completed" && isQuarterPassed(item.target_quarter);
  if (isOverdue) {
  ...
  const statusSvg = document.createElementNS(
  ...
  const planCategory = item.category || "Strategic:TIME";
  ...
  const badge = document.createElement("span");
  badge.className =
    "px-1.5 py-0.5 rounded text-[8px] font-black tracking-wide uppercase select-none cursor-pointer bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 shrink-0";
  ...
  const titleSpan = document.createElement("span");
  titleSpan.className =
    "font-semibold break-words whitespace-normal text-xs item-title-text flex-1 min-w-0" +
    (isReadOnly ? "" : " cursor-pointer");
  titleSpan.textContent = item.title;
  if (!isReadOnly) {
    titleSpan.onclick = () => startEditingItem(item, titleSpan);
  ...
  if (!isReadOnly) {
    const deleteBtn = document.createElement("button");
    deleteBtn.className =
      "text-red-400 hover:text-red-300 opacity-0 group-hover:opacity-100 transition-opacity font-bold px-1.5 py-0.5 rounded text-[10px] cursor-pointer";
    deleteBtn.innerHTML = "✕";
    deleteBtn.onclick = (e) => {
      e.stopPropagation();
      deleteItem(item.id);
  ...
  metaRow.className =
    "flex items-center justify-between gap-2.5 mt-1.5 pt-1.5 border-t border-[var(--border-color)] text-[10px] w-full min-w-0 select-none";
  ...
  targetSelect.className =
    "bg-[var(--bg-input)] hover:bg-[var(--bg-hover)] border border-[var(--border-color)] text-[9px] rounded-md px-1.5 py-0.5 font-bold text-[var(--text-primary)] focus:outline-none cursor-pointer transition-all";
  if (isReadOnly) {
  ...
  if (isOverdue) {
    ...
    overdueLabel.className =
      "text-[8px] font-black text-rose-500 bg-rose-500/10 border border-rose-500/20 px-1.5 py-0.5 rounded uppercase tracking-wider shrink-0 animate-pulse ml-1.5";
  ...
  if (linkedGaps.length > 0) {
    ...
    if (remainingCount > 0) {
      ...
      const remainingNames = linkedGaps
        .slice(maxVisible)
        .map((g) => g.title)
        .join(", ");
  ...
  if (!isReadOnly) {
    ...
    linkBtn.className =
      "text-[var(--text-secondary)] hover:text-[var(--text-primary)] font-bold px-1 py-0.5 rounded text-xs select-none hover:bg-[var(--bg-hover)] transition-all cursor-pointer";
    linkBtn.innerHTML = "+🔗";
...
function showLinkPopover(buttonEl, item, allGaps, linkedGapIds) {
  ...
  if (allGaps.length === 0) {
    ...
    document.getElementById("close-link-alert").onclick = () =>
      alertOverlay.remove();
    return;
  }
  ...
  header.className =
    "text-[10px] font-black uppercase tracking-wider text-[var(--text-secondary)] px-2 pb-1.5 mb-1.5 border-b border-[var(--border-color)]";
  ...
  allGaps.forEach((gap) => {
    ...
    span.className =
      "text-xs text-[var(--text-primary)] truncate font-semibold";
...
export function renderMySubmissions(submissions) {
  ...
  if (submissions.length === 0) {
    body.innerHTML =
      '<tr><td colspan="6" class="text-center py-6 text-gray-500 font-medium">No submission requests assigned to you.</td></tr>';
    return;
  }
  ...
  submissions.forEach((sub) => {
    ...
    const feedbackText = sub.feedback
      ? `<div class="bg-[var(--bg-input)] text-xs text-[var(--text-secondary)] px-3 py-1.5 rounded-lg border border-[var(--border-color)] max-w-xs truncate" title="${sub.feedback}">${sub.feedback}</div>`
      : `<span class="text-gray-500 italic text-[10px]">No feedback yet</span>`;
...
export function renderReviewsQueue(reviews) {
  ...
  if (reviews.length === 0) {
    body.innerHTML =
      '<tr><td colspan="7" class="text-center py-6 text-gray-500 font-medium">No reviews in your queue. Request a submission from your team members.</td></tr>';
    return;
  }
  ...
  Object.keys(groups).forEach((empId) => {
    ...
    headerRow.className =
      "bg-[var(--bg-input)]/55 border-b border-[var(--border-color)] select-none font-bold";
    headerRow.innerHTML = `
    ...
    group.items.forEach((rev) => {
      ...
      if (rev.status === "Needs Revision")
        statusColor = "text-rose-400 font-bold";
      if (rev.status === "Submitted") statusColor = "text-blue-400 font-bold";

      const feedbackText = rev.feedback
        ? `<div class="bg-[var(--bg-input)] text-xs text-[var(--text-secondary)] px-3 py-1.5 rounded-lg border border-[var(--border-color)] max-w-xs truncate" title="${rev.feedback}">${rev.feedback}</div>`
        : `<span class="text-gray-500 italic text-[10px]">-</span>`;
      const submittedOn = rev.submitted_at
        ? rev.submitted_at.split("T")[0]
        : `<span class="text-gray-500 italic text-[10px]">-</span>`;
      const programName = rev.dashboard_program || "General / Unlinked";

      const row = document.createElement("tr");
      row.className =
        "hover:bg-[var(--bg-hover)] transition-colors border-b border-[var(--border-color)] font-medium text-xs";
      row.innerHTML = `
...
export function populateReviewModal(review, data) {
  document.getElementById("review-modal-employee-role").textContent =
    `${review.employee_name} — ${review.employee_designation || review.employee_role}`;
  document.getElementById("review-modal-program-line").textContent =
    data.dashboard.program_line || "Not set";
  document.getElementById("review-modal-objective").textContent =
    data.dashboard.objective || "No objective logged.";
  document.getElementById("review-modal-notes").textContent =
    data.dashboard.notes || "No operational notes logged.";

  // Populate feedback text if any
  document.getElementById("review-comments-input").value =
    review.feedback || "";
  ...
  if (skills.length === 0) {
    rSkills.innerHTML =
      '<div class="text-gray-500 italic py-1 pl-1">None logged.</div>';
  } else {
    skills.forEach((item) => {
      rSkills.innerHTML += `
  ...
  if (training.length === 0) {
    ...
    training.forEach((item) => {
      ...
      const overdueBadge = isOverdue
        ? `<span class="text-[8px] font-black text-rose-400 bg-rose-500/10 px-1 py-0.5 rounded border border-rose-500/20 ml-1.5">⚠️ OVERDUE</span>`
        : "";
      const statusText = (item.status || "not_started")
        .replace("_", " ")
        .toUpperCase();
      const targetText = item.target_quarter
        ? `Target: ${item.target_quarter}`
        : "No Target Qtr";
      const completedText =
        item.status === "completed" && item.completed_quarter
          ? ` | Completed: ${item.completed_quarter}`
          : "";
...
export function renderTeam(team, currentUserId, currentUserRole) {
  ...
  team.forEach((emp) => {
    ...
    if (emp.hasDashboard) {
      ...
      if (emp.lastSubmissionStatus) {
        ...
        if (
          emp.lastSubmissionStatus === "Pending" ||
          emp.lastSubmissionStatus === "Submitted"
        ) {
  ...
  const awaitingReview = team.filter(
    (emp) =>
      emp.hasDashboard &&
      (emp.lastSubmissionStatus === "Pending" ||
        emp.lastSubmissionStatus === "Submitted"),
  ).length;
  const approvedPlans = team.filter(
    (emp) => emp.hasDashboard && emp.lastSubmissionStatus === "Approved",
  ).length;
  ...
  if (groupedEmployees.length === 0) {
    gridContainer.className =
      "flex justify-center items-center py-12 text-[var(--text-secondary)] w-full";
    gridContainer.innerHTML = `
  ...
  gridContainer.className =
    "w-full overflow-x-auto custom-scrollbar bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl shadow-lg";
  ...
  thead.className =
    "border-b border-[var(--border-color)] bg-[var(--bg-input)]/50 text-[9px] uppercase font-black tracking-wider text-[var(--text-secondary)] select-none";
  thead.innerHTML = `
    <tr>
  ...
  groupedEmployees.forEach((emp) => {
    ...
    const avatarGradient =
      emp.dashboards.length > 0
        ? "from-[var(--accent)]/30 to-indigo-500/20"
        : "from-slate-600/30 to-slate-500/20";

    // Counts
    const totalDashboards = emp.dashboards.length;
    const inReviewCount = emp.dashboards.filter(
      (d) =>
        d.lastSubmissionStatus === "Pending" ||
        d.lastSubmissionStatus === "Submitted",
    ).length;
    const reworkCount = emp.dashboards.filter(
      (d) => d.lastSubmissionStatus === "Needs Revision",
    ).length;
    const approvedCount = emp.dashboards.filter(
      (d) => d.lastSubmissionStatus === "Approved",
    ).length;

    // Parent Row
    const row = document.createElement("tr");
    row.className =
      "employee-row cursor-pointer hover:bg-[var(--bg-hover)] transition-colors align-middle font-medium border-b border-[var(--border-color)]";
    ...
    if (totalDashboards > 0) {
      dashboardsListHtml = emp.dashboards
        .map(
          (dash) => `
...
export function showViewerModal(data) {
  document.getElementById("viewer-employee-name").textContent =
    data.dashboard.name || "Employee Profile";
  document.getElementById("viewer-employee-role").textContent =
    `DASHBOARD #${data.dashboard.id.substring(0, 8)}`;
  document.getElementById("viewer-program-line").textContent =
    data.dashboard.program_line || "Not set";
  document.getElementById("viewer-objective").textContent =
    data.dashboard.objective || "No objective logged.";
  document.getElementById("viewer-status").textContent =
    data.dashboard.status || "On Track";
  document.getElementById("viewer-notes").textContent =
    data.dashboard.notes || "No operational notes logged.";
  ...
  if (skills.length === 0) {
    vSkills.innerHTML =
      '<div class="text-[var(--text-secondary)] italic py-1">None logged.</div>';
  } else {
    skills.forEach((item) => {
      vSkills.innerHTML += `
...
export function renderTreeNodes(container, list) {
  ...
  if (!list || list.length === 0) {
    container.innerHTML =
      '<div class="text-[var(--text-secondary)] italic py-1 pl-4 text-xs">No direct reports found.</div>';
    return;
  }
  ...
  list.forEach((emp) => {
    ...
    let statusBadge =
      '<span class="text-[9px] text-[var(--text-secondary)] italic px-2 py-0.5 border border-[var(--border-color)] rounded bg-[var(--bg-input)]">No Dashboard</span>';
    if (emp.hasDashboard) {
    ...
    const currentUserId = JSON.parse(
      sessionStorage.getItem("dashboard_user_data") || "{}",
    ).id;
...
export function renderOrgExplorer({
  ...
  const getPresence = (userId) => {
    ...
    if (status === "Active") {
      return {
        presenceClass: "presence-dot-green",
        presenceStatus: "Active Dashboard",
        statusText: "Active",
        colorClass: "text-emerald-400",
      };
    } else {
      return {
        presenceClass: "presence-dot-gray",
        presenceStatus: "No active dashboard",
        statusText: "No Dashboard",
...
export function showCustomAlert(message, title = "Notification") {
  return new Promise((resolve) => {
    ...
    overlay.className =
      "fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[9999] opacity-0 transition-opacity duration-300";
  ...
  return new Promise((resolve) => {
    ...
    if (submissions && submissions.length > 0) {
      submissions.forEach((sub) => {
        ...
        const submittedOn = sub.submitted_at
          ? sub.submitted_at.split("T")[0]
          : `<span class="text-gray-500 italic text-[9px]">Not submitted</span>`;
        const programName = sub.dashboard_program || "General / Unlinked";
      ...
      tableRows =
        '<tr><td colspan="6" class="text-center py-6 text-gray-500 font-medium">No submission requests found for this employee.</td></tr>';
    }
...
export function showSubmitDashboardModal(dashboards, currentDashboardId) {
  return new Promise((resolve) => {
    ...
    overlay.querySelector("#sub-plan-ok").onclick = () => {
      const dashboardId =
        overlay.querySelector("#sub-plan-dashboard").value || null;
      const dialog = overlay.querySelector("div");
      dialog.classList.remove("scale-100", "opacity-100");
      dialog.classList.add("scale-90", "opacity-0");
      overlay.classList.remove("opacity-100");
...
export function showSubmissionRequestModal(employeeName, dashboards) {
  return new Promise((resolve) => {
    ...
    overlay.querySelector("#sub-req-ok").onclick = () => {
      const dashboardId =
        overlay.querySelector("#sub-req-dashboard").value || null;
...
export function setupCustomAutocompletes() {
  const inputs = document.querySelectorAll(
    'input[list^="suggestions-"], input[id^="add-"]',
  );
  inputs.forEach((input) => {
    ...
    if (!box) {
      ...
      box.id = boxId;
      box.className =
        "custom-suggest-box fixed bg-[var(--bg-sidebar)] backdrop-blur-md border border-[var(--border-color)] text-[var(--text-primary)] rounded-xl shadow-2xl overflow-hidden hidden z-[99999] transition-all duration-200 opacity-0 scale-95 max-h-48 overflow-y-auto custom-scrollbar select-none";
  ...
  selects.forEach((select, selectIdx) => {
    ...
    if (
      ...
      if (!menu) {
        ...
        menu.id = menuId;
        menu.className =
          "custom-select-menu fixed bg-[var(--bg-sidebar)] backdrop-blur-md border border-[var(--border-color)] text-[var(--text-primary)] rounded-xl shadow-2xl overflow-hidden hidden z-[99999] transition-all duration-200 opacity-0 scale-95 max-h-56 overflow-y-auto custom-scrollbar select-none";
    ...
    trigger.type = "button";
    trigger.className =
      "custom-select-trigger w-full bg-[var(--bg-input)] border border-[var(--border-color)] text-[var(--text-primary)] rounded-lg px-3.5 py-2 text-xs font-semibold focus:outline-none focus:border-indigo-500 cursor-pointer flex items-center justify-between gap-2 select-none transition-all";
    ...
    arrow.className =
      "text-[9px] text-[var(--text-secondary)] transition-transform duration-200";
    arrow.innerHTML = "▼";
    ...
    trigger.onclick = (e) => {
      ...
      document
        .querySelectorAll(".custom-select-menu, .custom-suggest-box")
        .forEach((m) => {
          if (m.id !== menuId) {
      ...
      document
        .querySelectorAll(".custom-select-trigger span:last-child")
        .forEach((arrowEl) => {
...
export function renderHistory(deletedDashboards) {
  ...
  const renderTo = (parent, isDrawer) => {
    ...
    deletedDashboards.forEach((d) => {
      const deletedDate = d.deleted_at
      ...
      chevron.className =
        "accordion-chevron text-[10px] text-gray-500 transform transition-transform";
      chevron.innerHTML = "▶";
      ...
      name.className =
        "font-bold text-[11px] truncate text-[var(--text-primary)]";
      ...
      delTag.className =
        "px-1.5 py-0.5 rounded text-[8px] font-black tracking-wide uppercase bg-red-500/10 border border-red-500/20 text-red-400 shrink-0";
      ...
      restoreBtn.className =
        "px-2 py-1 rounded bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 text-indigo-400 font-bold transition-all cursor-pointer text-[9px]";
      ...
      destroyBtn.className =
        "px-2 py-1 rounded bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-red-400 font-bold transition-all cursor-pointer text-[9px]";
      ...
      if (versions.length === 0) {
        ...
        versions.forEach((v) => {
          ...
          row.className =
            "flex items-center justify-between p-2 mb-1.5 rounded bg-[var(--bg-input)] border border-[var(--border-color)] text-[10px] hover:border-indigo-500/30 transition-all";
...
export function renderVersions(activeDashboardsWithVersions) {
  ...
  const renderTo = (parent, isDrawer) => {
    ...
    if (
      !activeDashboardsWithVersions ||
      activeDashboardsWithVersions.length === 0
    ) {
      parent.innerHTML = `
        <div class="flex flex-col items-center justify-center p-8 bg-[var(--bg-input)] rounded-xl border border-[var(--border-color)] text-center text-[var(--text-secondary)]">
    ...
    activeDashboardsWithVersions.forEach((d) => {
      ...
      if (isCurrentActive) {
        ...
        activePill.className =
          "px-1.5 py-0.5 rounded text-[8px] font-black tracking-wide uppercase bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 shrink-0";
      ...
      saveBtn.className =
        "px-2 py-1.5 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 hover:bg-indigo-500/20 transition-all font-bold text-[9px] cursor-pointer flex items-center justify-center gap-1 shadow-sm";
      ...
      if (
        window.currentDashboardUserId === window.userData?.id &&
        !d.is_deleted
      ) {
      ...
      if (versions.length === 0) {
        ...
        versions.forEach((v) => {
          const row = document.createElement("div");
          row.className =
            "flex items-center justify-between p-2 mb-1.5 rounded bg-[var(--bg-input)] border border-[var(--border-color)] text-[10px] hover:border-[var(--accent)]/30 transition-all";
          ...
          const restoreBtn = document.createElement("button");
          restoreBtn.className =
            "px-2 py-1 rounded bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 text-emerald-400 font-bold transition-all cursor-pointer text-[9px]";
          restoreBtn.textContent = "Restore";
          restoreBtn.onclick = () => {
