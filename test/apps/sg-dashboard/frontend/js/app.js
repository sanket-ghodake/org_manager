...
export function toggleTeamViewMode(mode) {
  ...
  if (mode === "list") {
    ...
    btnList.className =
      "px-4 py-1.5 rounded-lg text-xs font-bold transition-all bg-[var(--bg-hover)] text-[var(--text-primary)] shadow-sm";
    btnTree.className =
      "px-4 py-1.5 rounded-lg text-xs font-bold transition-all text-[var(--text-secondary)] hover:text-[var(--text-primary)]";
  } else if (mode === "tree") {
...
export async function updateDashboardNotes() {
  const notes = document
    .getElementById("dashboard-notes-textarea")
    .value.trim();
...
export async function cyclePlanBadgeType(item, planType, currentBadge) {
  ...
  const nextBadge =
    planType === "Strategic"
      ? strategicFlow[currentBadge]
      : tacticalFlow[currentBadge];
...
export function startEditingItem(item, containerSpan) {
  ...
  const titleTextSpan =
    containerSpan.querySelector(".item-title-text") || containerSpan;
  const originalText = titleTextSpan.textContent;

  const input = document.createElement("input");
  input.type = "text";
  input.className =
    "bg-[var(--bg-input)] border border-[var(--accent)] text-[var(--text-primary)] rounded text-xs px-2 py-0.5 font-medium w-full focus:outline-none";
...
export async function updatePlanTargetQuarter(itemId, newTargetQuarter) {
  try {
    await api.updateDashboardItem(apiToken, itemId, {
      target_quarter: newTargetQuarter,
    });
    await loadMyDashboard();
  } catch (err) {
    console.error(err);
...
export async function updatePlanCompletedQuarter(itemId, newCompletedQuarter) {
  try {
    await api.updateDashboardItem(apiToken, itemId, {
      completed_quarter: newCompletedQuarter,
    });
    await loadMyDashboard();
  } catch (err) {
    console.error(err);
...
export async function updateItemLinks(sourceId, targetIds) {
  try {
    await api.syncDashboardItemLinks(
      apiToken,
      currentDashboardId,
      sourceId,
      targetIds,
    ...
    await ui.showCustomAlert(
      "Failed to update linked skill gaps: " + err.message,
      "Error",
    );
  }
}
...
export async function submitDashboard(id, linkedDashboardId) {
  ...
  if (!dashboardId) {
    ...
    const modalResult = await ui.showSubmitDashboardModal(
      dashboards,
      currentDashboardId,
    );
    ...
    const confirmed = await ui.showCustomConfirm(
      `Are you sure you want to submit your Technical Resource Plan for "${progName}" for review?`,
      "Submit Dashboard",
    );
    if (!confirmed) return;
  }
  ...
  try {
    ...
    console.error(err);
    await ui.showCustomAlert(
      "Failed to submit dashboard: " + err.message,
      "Error",
    );
  }
}
...
export async function freezeSubmission(requestId) {
  const confirmed = await ui.showCustomConfirm(
    "Are you sure you want to freeze this submission request? No further edits will be allowed and the employee's last active dashboard version will be submitted directly.",
    "Freeze Submission",
  );
  if (!confirmed) return;
  try {
    ...
    await ui.showCustomAlert(
      "Submission request frozen and locked successfully!",
      "Success",
    );
    await loadSubmissions();
    if (userData.role === "Manager" || userData.role === "Admin") {
    ...
    console.error(err);
    await ui.showCustomAlert(
      "Failed to freeze submission: " + err.message,
      "Error",
    );
  }
}
...
export async function triggerRequestSubmission(empId) {
  ...
  const modalResult = await ui.showRequestSubmissionModal(
    employeeName,
    dashboards,
  );
  if (!modalResult) return;
  ...
  try {
    const res = await api.createSubmissionRequest(
      apiToken,
      empId,
      deadline,
      dashboardId,
    ...
    if (res.ok) {
      await ui.showCustomAlert(
        "Submission request generated successfully!",
        "Success",
      );
      await loadTeamView();
      await loadSubmissions();
    ...
    console.error(err);
    await ui.showCustomAlert(
      "Failed to request submission: " + err.message,
      "Error",
    );
  }
}
...
export async function changeActiveEmployeeContext(
  userId,
  selectedDashboardId = "",
) {
  ...
  if (
    ...
    currentDashboardUserId !== targetUserId
  ) {
    const activeReview = window.cachedReviews
      ? window.cachedReviews.find((r) => r.id === activeReviewId)
      : null;
    if (activeReview && activeReview.employee_id !== targetUserId) {
      ui.showCustomAlert(
        "Please close or submit the current review panel before switching to another user's workspace.",
        "Review Mode Active",
      );
      return;
    }
  ...
  try {
    ...
    if (overlay) {
      ...
      if (title) {
        title.textContent = isInitialLoad
          ? "Loading Your Workspace..."
          : "Switching Workspace Context...";
      }
      if (subtitle) {
        subtitle.textContent = isInitialLoad
          ? "Initializing and syncing goal dashboard from database..."
          : "Synchronizing goal dashboard and reporting structures...";
      }
    }
    ...
    const teamPromise = api
      .fetchTeam(apiToken, currentDashboardUserId)
      .catch((e) => {
        console.warn("Failed to load team data in context switch:", e);
        return { team: [] };
      });

    const submissionsPromise = api
      .fetchSubmissions(apiToken, currentDashboardUserId)
      .catch((e) => {
        console.warn("Failed to load submissions in context switch:", e);
        return { submissions: [] };
      });

    const dashboardsListPromise = api
      .fetchDashboards(apiToken, currentDashboardUserId, includeDeleted)
      .catch((e) => {
    ...
    const ownerName = dashData.dashboard.name || userData.name;
    const ownerRole = dashData.dashboard.role || userData.role;
    ...
    if (!isSelf && dashData.dashboard.name) {
      const emp = {
        ...
        managerId:
          dashData.dashboard.manager_id || dashData.dashboard.managerId || null,
    ...
    const isLocked =
      lastSubStatus === "Submitted" ||
      lastSubStatus === "Approved" ||
      lastSubStatus === "Pending";
      ...
      if (isReadOnly && !isDeleted) {
        ...
        if (isLocked && isSelf) {
          warningBanner.className =
            "bg-indigo-500/10 border-b border-indigo-500/20 text-indigo-400 px-6 py-2 text-xs font-semibold flex items-center justify-between gap-2 select-none";
          ...
          if (window.cachedReviews) {
            const matchingReview = window.cachedReviews.find(
              (r) =>
                r.employee_id === currentDashboardUserId &&
                r.dashboard_id === currentDashboardId,
            );
            if (matchingReview) {
          ...
          warningBanner.className =
            "bg-amber-500/10 border-b border-amber-500/20 text-amber-500 px-6 py-2 text-xs font-medium flex items-center justify-between gap-2 select-none";
          warningBanner.innerHTML = `
            <div class="flex items-center gap-2">
              <span>⚠️</span>
      ...
      if (activeDashboards.length > 0) {
        ...
        activeDashboards.forEach((d) => {
          ...
          if (d.last_submission_status === "Approved") {
            if (
              d.last_submission_date &&
              d.updated_at &&
              new Date(d.updated_at) > new Date(d.last_submission_date)
            ) {
    ...
    if (addBtn) {
      if (isReadOnly && !isDeleted) addBtn.classList.add("hidden");
        ...
        addBtn.onclick = async () => {
          const newName = await ui.showCustomPrompt(
          ...
          try {
            ...
            await changeActiveEmployeeContext(
              currentDashboardUserId,
              res.newDashboardId,
            );
          } catch (e) {
            await ui.showCustomAlert(
              "Failed to add program: " + e.message,
              "Error",
            );
          }
        };
      ...
      if (isReadOnly) renameBtn.classList.add("hidden");
        ...
        renameBtn.onclick = async () => {
          const newName = await ui.showCustomPrompt(
            ...
            currentDashboardData.program_line || "",
            "Rename Program",
          );
          if (
            newName &&
            newName.trim() &&
            newName.trim() !== currentDashboardData.program_line
          ) {
            ...
            try {
              ...
              await ui.showCustomAlert(
                "Failed to rename program: " + e.message,
                "Error",
              );
            }
          }
    ...
    if (duplicateBtn) {
      if (isReadOnly) duplicateBtn.classList.add("hidden");
        ...
        duplicateBtn.onclick = async () => {
          const confirmed = await ui.showCustomConfirm(
            "Are you sure you want to duplicate the active program?",
            "Duplicate Program",
          );
          if (!confirmed) return;
          try {
            const res = await api.duplicateDashboard(
              apiToken,
              currentDashboardId,
            );
            await ui.showCustomAlert(
              `Program duplicated as: ${res.newProgramName}`,
              "Success",
            );
            await changeActiveEmployeeContext(
              currentDashboardUserId,
            ...
            await ui.showCustomAlert(
              "Failed to duplicate program: " + e.message,
              "Error",
            );
          }
        };
    ...
    if (deleteBtn) {
      if (isReadOnly) deleteBtn.classList.add("hidden");
        ...
        deleteBtn.onclick = async () => {
          const activeList =
            dashboardsData.dashboards.filter((d) => !d.is_deleted) || [];
          if (activeList.length <= 1) {
            await ui.showCustomAlert(
              "Cannot delete the only active dashboard program. You must maintain at least one active program.",
              "Action Restricted",
            );
            return;
          }
          const confirmed = await ui.showCustomConfirm(
            "Are you sure you want to archive/delete the active program? You can restore it using the Show Trash filter next to the selector.",
            "Archive Program",
          );
          if (!confirmed) return;
          try {
      ...
      exportBtn.onclick = async () => {
        try {
          ...
          const exportData = {
            ...
            skills: items
              .filter((i) => i.section === "key_skill")
              .map((i) => ({ title: i.title, priority: i.category })),
            gaps: items
              .filter((i) => i.section === "gap")
              .map((i) => ({ title: i.title, priority: i.category })),
            training_plans: items
              .filter((i) => i.section === "training_plan")
              .map((i) => ({ title: i.title, priority: i.category })),
          };
          ...
          downloadAnchor.setAttribute(
            "download",
            `${ownerName.replace(/\s+/g, "_")}_${exportData.program.replace(/\s+/g, "_")}.json`,
          );
          ...
          await ui.showCustomAlert(
            "Failed to export program: " + e.message,
            "Error",
          );
        }
      };
    ...
    if (subProg) {
      subProg.textContent =
        dashData.dashboard.program_line || "Default Program";
    }

    const objDisplay = document.getElementById("sub-objective-display");
    if (objDisplay) {
      objDisplay.textContent =
        dashData.dashboard.objective ||
        (isReadOnly ? "No objective stated." : "Click to enter objective");
    ...
    updateAllSuggestions().catch((err) =>
      console.warn("Failed to update suggestions:", err),
    );

    // Populate submissions using pre-fetched data to avoid duplicate network queries
    await loadSubmissions(
      currentDashboardUserId,
      subData.submissions || [],
    ...
    if (teamViewTitle) {
      teamViewTitle.textContent = isSelf
        ? "👥 My Team Management"
        : `👥 Team Management for ${ownerName}`;
    }
    if (teamViewSubtitle) {
    ...
    try {
      sessionStorage.setItem(
        "dashboard_active_employee_id",
        currentDashboardUserId,
      );
      sessionStorage.setItem(
        "dashboard_active_dashboard_id",
        currentDashboardId,
      );
    } catch (e) {
    ...
    await ui.showCustomAlert(
      `Failed to load workspace context: ${err.message}`,
      "Error",
    );
...
export async function viewEmployeeSubmissions(empId) {
  ...
  try {
    ...
    console.error(err);
    await ui.showCustomAlert(
      "Failed to load employee submissions: " + err.message,
      "Error",
    );
  }
}
...
function toggleWorkspaceEditState(isReadOnly) {
  ...
  if (subObjDisp) {
    if (isReadOnly) {
      subObjDisp.classList.remove(
        "cursor-pointer",
        "hover:text-[var(--text-primary)]",
      );
      subObjDisp.removeAttribute("onclick");
    } else {
      subObjDisp.classList.add(
        "cursor-pointer",
        "hover:text-[var(--text-primary)]",
      );
...
function updateSyncStatusUI(status) {
  ...
  if (status === "syncing") {
    syncEl.className =
      "flex items-center gap-1.5 ml-2 text-[9px] font-bold text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-full select-none";
    syncEl.innerHTML = `
    ...
    syncEl.className =
      "flex items-center gap-1.5 ml-2 text-[9px] font-bold text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded-full select-none";
    syncEl.innerHTML = `
    ...
    syncEl.className =
      "flex items-center gap-1.5 ml-2 text-[9px] font-bold text-rose-400 bg-rose-500/10 border border-rose-500/20 px-2 py-0.5 rounded-full select-none";
    syncEl.innerHTML = `
    ...
    // live
    syncEl.className =
      "flex items-center gap-1.5 ml-2 text-[9px] font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full select-none";
    syncEl.innerHTML = `
...
async function loadSubmissions(
  ...
  preFetchedDashboards = null,
) {
  try {
    const targetUserId = userId || currentDashboardUserId || userData.id;
    const isSelf = targetUserId === userData.id;

    // Fetch submissions requests for the target employee if not pre-fetched
    const submissions =
      preFetchedSubmissions !== null
        ? preFetchedSubmissions
        : (await api.fetchSubmissions(apiToken, targetUserId)).submissions ||
          [];

    // Fetch all active dashboards of target employee if not pre-fetched
    const dashboards =
      preFetchedDashboards !== null
        ? preFetchedDashboards
        : (await api.fetchDashboards(apiToken, targetUserId, false))
            .dashboards || [];
...
export function switchSubmissionsTab(subTab) {
  ...
  if (subTab === "my-submissions") {
    ...
    btnMy.className =
      "px-4 py-2 rounded-lg text-xs font-bold transition-all bg-[var(--bg-hover)] text-[var(--text-primary)] shadow-sm";
    btnReviews.className =
      "px-4 py-2 rounded-lg text-xs font-bold transition-all text-[var(--text-secondary)] hover:text-[var(--text-primary)]";
...
export function filterReviewsQueue() {
  const q =
    document.getElementById("review-search-input")?.value.toLowerCase() || "";
  const status =
  ...
  const filtered = window.cachedReviews.filter((rev) => {
    const matchName =
      ...
      rev.employee_email.toLowerCase().includes(q);
...
export async function openReviewModal(requestId) {
  ...
  if (
    ...
    await ui.showCustomAlert(
      "Access Denied: Only the author and the respective manager are authorized to access this review pane.",
      "Unauthorized",
    );
    return;
  }
  ...
  try {
    ...
    console.error(err);
    await ui.showCustomAlert(
      "Failed to load dashboard data for review: " + err.message,
      "Error",
    );
  } finally {
...
export async function submitReviewAction(status) {
  ...
  const feedback =
    document.getElementById("review-comments-input")?.value.trim() || "";

  const confirmed = await ui.showCustomConfirm(
    `Are you sure you want to set this submission status to "${status}"?`,
    "Submit Review",
  );
  if (!confirmed) return;
  ...
  try {
    ...
    closeReviewModal();
    await ui.showCustomAlert(
      `Submission successfully updated to: ${status}`,
      "Success",
    );
    await loadSubmissions();
  } catch (err) {
    console.error(err);
    await ui.showCustomAlert(
      "Failed to submit review decision: " + err.message,
      "Error",
    );
  }
}
...
export function openSideReviewPanel(review) {
  ...
  document.getElementById("side-review-employee-name").textContent =
    review.employee_name;
  document.getElementById("side-review-employee-role").textContent =
    review.employee_designation || review.employee_role;
  document.getElementById("side-review-program-line").textContent =
    review.dashboard_program || "Selected Program";
  ...
  if (deadlineEl) {
    deadlineEl.textContent = review.deadline
  ...
  if (stateCard && stateIcon && stateTitle && stateDesc) {
    if (review.status === "Pending") {
      stateCard.className =
        "p-3.5 rounded-xl border border-amber-500/20 bg-amber-500/5 flex flex-col space-y-1.5 shadow-sm";
      stateIcon.textContent = "⏳";
      stateTitle.textContent = "Awaiting Submission";
      stateTitle.className =
        "text-[10px] font-black uppercase tracking-wider text-amber-500";
      stateDesc.textContent =
        "The author needs to finalize their dashboard and submit it for review. Workspace editing is open for the employee.";
    } else if (review.status === "Submitted") {
      stateCard.className =
        "p-3.5 rounded-xl border border-indigo-500/20 bg-indigo-500/5 flex flex-col space-y-1.5 shadow-sm";
      stateIcon.textContent = "📝";
      stateTitle.textContent = "Review Required";
      stateTitle.className =
        "text-[10px] font-black uppercase tracking-wider text-indigo-400";
      stateDesc.textContent =
        "Your review is required! Please verify the key skills, gaps, and training plan items on the left. Log comments and choose to Approve or Request Revision.";
    } else if (review.status === "Needs Revision") {
      stateCard.className =
        "p-3.5 rounded-xl border border-rose-500/20 bg-rose-500/5 flex flex-col space-y-1.5 shadow-sm";
      stateIcon.textContent = "⚠️";
      stateTitle.textContent = "Rework in Progress";
      stateTitle.className =
        "text-[10px] font-black uppercase tracking-wider text-rose-400";
      stateDesc.textContent =
        "Revision requested. Awaiting employee to apply comments and re-submit. Workspace editing permissions have been reopened for the employee.";
    } else if (review.status === "Approved") {
      stateCard.className =
        "p-3.5 rounded-xl border border-emerald-500/20 bg-emerald-500/5 flex flex-col space-y-1.5 shadow-sm";
      stateIcon.textContent = "✓";
      stateTitle.textContent = "Approved & Finalized";
      stateTitle.className =
        "text-[10px] font-black uppercase tracking-wider text-emerald-400";
      stateDesc.textContent =
        "This program plan has been approved and signed off. Editing permissions are locked.";
    } else {
      stateCard.className =
        "p-3.5 rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)] flex flex-col space-y-1.5 shadow-sm";
      stateIcon.textContent = "❓";
      stateTitle.textContent = "Unknown State";
      stateTitle.className =
        "text-[10px] font-black uppercase tracking-wider text-[var(--text-primary)]";
  ...
  const btnRevision = document.getElementById(
    "side-review-action-revision-btn",
  );
...
export async function submitSideReview(status) {
  if (!activeReviewId) return;
  const feedback =
    document.getElementById("side-review-comments-input")?.value.trim() || "";
...
async function loadTeamView(userId) {
  ...
  if (teamViewTitle) {
    teamViewTitle.textContent = isSelf
      ? "👥 My Team Management"
      : `👥 Team Management for ${targetUserName}`;
  }
  if (teamViewSubtitle) {
    teamViewSubtitle.textContent = isSelf
      ? "Monitor development plans, audit health states, and trigger submission request workflows for your reporting chain."
  ...
  try {
    ...
    try {
      const cleanUrl =
        ...
        window.location.pathname.replace("/callback", "");
    ...
    ui.showAuthError(
      "Federated session expired or invalid. Redirecting to Single Sign-On...",
    );
    setTimeout(() => {
      redirectToSSO();
    }, 2000);
...
async function performBackgroundSync() {
  ...
  try {
      ...
      if (currentDashboardUserId && currentDashboardId) {
        ...
        if (dashData && dashData.dashboard) {
          ...
          if (dashData.dashboard.updated_at !== lastDashboardUpdatedAt) {
            console.log(
              `[Smart Sync] Detected dashboard updates (last updated: ${dashData.dashboard.updated_at}). Updating UI...`,
            );
            ...
            if (
              objDisplay &&
              (!objInput || objInput.classList.contains("hidden"))
            ) {
              objDisplay.textContent =
                dashData.dashboard.objective ||
                (isReadOnly
                  ? "No objective stated."
                  : "Click to enter objective");
              if (!dashData.dashboard.objective) {
                objDisplay.classList.add("italic");
              } else {
                objDisplay.classList.remove("italic");
    ...
    if (teamTab && !teamTab.classList.contains("hidden")) {
      const targetUserId = currentDashboardUserId || userData.id;
      const targetUser =
        cachedUsers.find((u) => u.id === targetUserId) || userData;
      const targetUserRole = targetUser ? targetUser.role : "Employee";
...
function startBackgroundSync() {
  ...
  async function syncLoop() {
    ...
    if (document.visibilityState !== "visible") {
      ...
      const isSelf =
        !currentDashboardUserId || currentDashboardUserId === userData.id;
...
async function initializeApplication() {
  ...
  document.getElementById("user-role").textContent =
    userData.designation || userData.role.replace("_", " ");
  document.getElementById("user-avatar").textContent = userData.name
    .split(" ")
    .map((n) => n[0])
    .join("");
  ...
  // Sync current user session in background without blocking initial workspace load
  api
    .syncUserDirectory(apiToken, [userData])
    .then(() =>
      console.log("User profile synced inside local SQLite DB successfully."),
    )
    .catch((err) => console.warn("Failed to sync current user profile:", err));
  ...
  const savedDashboardId = sessionStorage.getItem(
    "dashboard_active_dashboard_id",
  );
  ...
  const tabToRestore =
    savedTab && validTabs.includes(savedTab) ? savedTab : "my-dashboard";

  const validTeamModes = ["list", "tree"];
  const teamModeToRestore =
    savedTeamMode && validTeamModes.includes(savedTeamMode)
      ? savedTeamMode
      : "list";

  const targetEmployeeId =
    savedEmployeeId &&
    typeof savedEmployeeId === "string" &&
    savedEmployeeId.trim()
      ? savedEmployeeId.trim()
      : userData.id;
  const targetDashboardId =
    savedDashboardId &&
    typeof savedDashboardId === "string" &&
    savedDashboardId.trim()
      ? savedDashboardId.trim()
      : "";
...
async function initSession() {
  ...
  if (window.self !== window.top) {
    console.log(
      "App running inside iframe, waiting for postMessage auth handshake from parent...",
    );
    setTimeout(() => {
      if (!apiToken) {
        ui.showAuthError(
          "Handshake timeout: Did not receive secure credentials from parent portal.",
        );
      }
    }, 5000);
    return;
  ...
  if (codeFromUrl) {
    try {
      localStorage.removeItem("dashboard_logged_out");
    } catch (e) {}
  ...
  let loggedOut = false;
  try {
    loggedOut = localStorage.getItem("dashboard_logged_out") === "true";
  } catch (e) {}

  if (loggedOut) {
    console.log("User explicitly logged out. Displaying Sign-In prompt.");
    showLoggedOutScreen();
    return;
  }
...
async function showLoggedOutScreen() {
  const overlay = document.getElementById("auth-overlay");
  if (overlay) {
    overlay.classList.remove("hidden");
    ...
    try {
      const config = await api.fetchConfig();
      if (config && config.portalSsoUrl) {
        const ssoUrlObj = new URL(config.portalSsoUrl);
        const portalBaseUrl = ssoUrlObj.origin;
        portalLinkHtml = `
          <div class="text-[10px] text-[var(--text-secondary)] mt-1">
            Need to switch accounts? <a href="${portalBaseUrl}" target="_blank" rel="noopener noreferrer" class="text-indigo-400 hover:underline font-bold">Open Parent Portal</a>
          </div>
        `;
      }
    } catch (e) {
      console.warn("Failed to fetch portal base URL for switch accounts link:", e);
    }

    overlay.innerHTML = `
      <div class="bg-[var(--bg-card)] p-8 rounded-2xl flex flex-col items-center gap-6 max-w-sm text-center border border-[var(--border-color)] shadow-2xl animate-fade-in">
        <div class="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl bg-gradient-to-tr from-purple-600 to-pink-500 text-white shadow-lg shadow-purple-500/20">
          📊
        </div>
        <div class="space-y-2">
          <h3 class="font-bold text-base text-white">Signed Out</h3>
          <p class="text-xs text-[var(--text-secondary)] leading-relaxed">
            You have been signed out of your SG Dashboard workspace session.
          </p>
        </div>
        <button onclick="handleSignIn()" class="w-full py-2.5 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white transition-all shadow-md shadow-indigo-600/10 active:scale-[0.98] cursor-pointer">
          Sign In Again
        </button>
        ${portalLinkHtml}
      </div>
...
window.handleSignIn = function () {
  ...
  if (overlay) {
    overlay.innerHTML = `
      <div class="bg-[var(--bg-card)] p-8 rounded-2xl flex flex-col items-center gap-4 max-w-sm text-center border border-[var(--border-color)] shadow-2xl">
        <div class="w-12 h-12 rounded-full border-4 border-t-[var(--accent)] border-white/10 animate-spin"></div>
        <h3 class="font-bold text-sm text-white">Redirecting to SSO</h3>
        <p class="text-[11px] text-[var(--text-secondary)]">Connecting to central authentication service...</p>
      </div>
    `;
  }
  redirectToSSO();
...
export function renderSearchResults(employees, resultsContainer, inputEl) {
  ...
  employees.forEach((emp) => {
    ...
    item.className =
      "search-result-item flex items-center justify-between p-2.5 hover:bg-[var(--bg-hover)] cursor-pointer border-b border-[var(--border-color)] last:border-b-0 transition-colors";
    item.onclick = () => {
      resultsContainer.classList.add("hidden");
      inputEl.value = "";
...
export async function handleHeaderSearch(inputEl) {
  ...
  if (directoryFetched && cachedUsers.length > 0) {
    ...
    const employees = cachedUsers.filter(
      (u) =>
...
export async function toggleHierarchyNode(userId) {
  ...
  try {
    ...
    if (reports.length === 0) {
      container.innerHTML =
        '<div class="text-[var(--text-secondary)] italic py-1 pl-4 text-xs">No direct reports.</div>';
    ...
    toggleBtn.textContent = "▶";
    await ui.showCustomAlert(
      "Failed to load reporting structure: " + err.message,
      "Error",
    );
  }
}
...
export async function loadOrgExplorer(focusedUserId) {
  ...
  try {
    ...
    if (!focusedUser) {
      ...
      try {
        ...
        focusedUser = {
          ...
          role: dashData.dashboard.role,
          managerId:
            dashData.dashboard.manager_id || dashData.dashboard.managerId,
          designation: dashData.dashboard.designation || "",
        };
        ...
        console.error(
          "Focused user not found and failed to fetch details:",
          err,
        );
      }
    }
    ...
    const peers = focusedUser.managerId
      ? cachedUsers.filter(
          (u) =>
            u.managerId === focusedUser.managerId && u.id !== focusedUser.id,
        )
      : [];

    // 5. Build Direct Reports (reporting to focused user)
    const directReports = cachedUsers.filter(
      (u) => u.managerId === focusedUser.id,
    );
...
export function handleLogout() {
  try {
    sessionStorage.clear();
    localStorage.setItem("dashboard_logged_out", "true");
  } catch (e) {}
  // Redirect to clean base URL without query parameters/OAuth code
...
export function showProfilePopup(event) {
  ...
  overlay.className =
    "fixed inset-0 bg-black/60 backdrop-blur-sm z-[99998] transition-opacity duration-200 opacity-0";

  // Create modal content
  const modal = document.createElement("div");
  modal.id = "user-profile-popup";
  modal.className =
    "fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[var(--bg-sidebar)] border border-[var(--border-color)] text-[var(--text-primary)] rounded-3xl shadow-2xl p-6 w-80 max-w-[90%] z-[99999] transition-all duration-200 opacity-0 scale-95 select-none";
...
export async function saveCurrentVersion(dashboardId = null) {
  ...
  if (!targetId) {
    await ui.showCustomAlert(
      "No active dashboard program loaded to save a version of.",
      "Action Restricted",
    );
    return;
  }
  ...
  if (!isSelf) {
    await ui.showCustomAlert(
      "You cannot save version snapshots of a read-only workspace.",
      "Action Restricted",
    );
    return;
  }
  if (
    targetId === currentDashboardId &&
    currentDashboardData &&
    currentDashboardData.is_deleted
  ) {
    await ui.showCustomAlert(
      "You cannot save version snapshots of a deleted workspace.",
      "Action Restricted",
    );
    return;
  }
  ...
  try {
    ...
    await ui.showCustomAlert(
      "Dashboard version saved successfully!",
      "Success",
    );
...
export async function restoreVersionAction(
  ...
  const confirmed = await ui.showCustomConfirm(
    `Are you sure you want to restore this dashboard version "${versionName}"? Current active goals and links will be overwritten with the snapshot.`,
    "Restore Version",
  );
  if (!confirmed) return;
  try {
    ...
    await ui.showCustomAlert(
      `Dashboard restored to version "${versionName}" successfully.`,
      "Success",
    );
    await changeActiveEmployeeContext(currentDashboardUserId, dashboardId);
  } catch (err) {
    await ui.showCustomAlert(
      "Failed to restore version: " + err.message,
      "Error",
    );
  }
}
...
export async function deleteVersionAction(dashboardId, versionId) {
  const confirmed = await ui.showCustomConfirm(
    "Are you sure you want to delete this version snapshot permanently?",
    "Delete Version",
  );
  if (!confirmed) return;
  try {
    ...
    await ui.showCustomAlert(
      "Failed to delete version: " + err.message,
      "Error",
    );
  }
}
...
export async function restoreDashboardAction(dashboardId) {
  try {
    ...
    await ui.showCustomAlert(
      "Dashboard program restored successfully.",
      "Success",
    );
...
export async function deleteDashboardPermanentAction(dashboardId) {
  const confirmed = await ui.showCustomConfirm(
    "Are you sure you want to permanently destroy this dashboard program? All items, links, and history for it will be lost forever.",
    "Destroy Program Permanently",
  );
  if (!confirmed) return;
  try {
    await api.deleteDashboardPermanent(apiToken, dashboardId);
    await ui.showCustomAlert(
      "Dashboard program permanently deleted.",
      "Success",
    );
    ...
    await ui.showCustomAlert(
      "Failed to delete program permanently: " + err.message,
      "Error",
    );
  }
}
...
export async function loadHistoryAndVersions() {
  ...
  if (pageSaveVerBtn) {
    const isSelf = currentDashboardUserId === userData.id;
    const isDeleted = currentDashboardData
      ? !!currentDashboardData.is_deleted
      : false;
    if (!isSelf || isDeleted) {
  ...
  try {
    ...
    const dashData = await api.fetchDashboards(
      apiToken,
      currentDashboardUserId,
      false,
    );
    ...
    const dashboardsWithVersions = await Promise.all(
      dashboards.map(async (d) => {
        try {
          ...
          console.warn(
            "Failed to fetch versions for deleted dashboard:",
            d.id,
            e,
          );
          return { ...d, versions: [] };
...
export async function restoreDeletedDashboardToVersion(
  dashboardId,
  versionId,
  versionName,
) {
  const confirmed = await ui.showCustomConfirm(
    `Restore dashboard program to snapshot "${versionName}"? This will also reactivate the program.`,
    "Restore Version",
  );
  if (!confirmed) return;
  try {
    ...
    await api.restoreVersion(apiToken, dashboardId, versionId);
    await ui.showCustomAlert(
      `Dashboard program restored and reset to version "${versionName}"!`,
      "Success",
    );
...
export function switchDrawerTab(tab) {
  ...
  if (tab === "active") {
    btnActive.className =
      "flex-1 py-1.5 rounded-lg text-xs font-bold transition-all bg-[var(--bg-hover)] text-[var(--text-primary)] shadow-sm";
    btnTrash.className =
      "flex-1 py-1.5 rounded-lg text-xs font-bold transition-all text-[var(--text-secondary)] hover:text-[var(--text-primary)]";
