...
export async function getWorkingPortalUrl(): Promise<string> {
  if (process.env.PORTAL_INTERNAL_URL) {
    return process.env.PORTAL_INTERNAL_URL;
  }
  ...
  activeResolvePromise = (async () => {
    ...
    for (const res of results) {
      if (res.status === "fulfilled") {
        ...
        console.log(
          `Resolved and cached working portal URL: ${cachedWorkingUrl}`,
        );
    ...
    lastCheckedTime = Date.now();
    console.log(
      `All candidates unreachable. Caching fallback PORTAL_INTERNAL_URL: ${cachedWorkingUrl} for 30s`,
    );
    activeResolvePromise = null;
