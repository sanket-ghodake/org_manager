...
export default async function dashboardRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (request: any, reply) => {
      ...
      const isAuthorized =
        ...
        user.role === "Admin" ||
        (await checkUplineManager(user.id, targetUserId));

      if (!isAuthorized) {
        return reply.status(403).send({
          error:
            "Forbidden: You are not authorized to view this user's dashboards.",
        });
      }
  ...
  // Get Specified Dashboard (View mode open to authorized users)
  fastify.get(
    ...
    async (request: any, reply) => {
      ...
      if (!isAuthorized) {
        return reply.status(403).send({
          error:
            "Forbidden: You are not authorized to view this user's dashboard.",
        });
      }
          ...
          if (userCheck.rows.length === 0) {
            ...
            try {
              ...
              if (pRes.ok) {
                ...
                const found = pData.users?.find(
                  (u: any) => u.id === targetUserId,
                );
                if (found) {
                  _dummyName = found.name;
                  dummyRole =
                    found.role === "admin" || found.role === "super_admin"
                      ? "Admin"
                      : found.role === "manager"
                        ? "Manager"
                        : "Employee";
              ...
              console.warn(
                "Failed to auto-fetch missing user on dashboard query:",
                err,
              );
            }

            // If still not inserted, return 404 instead of generating a dummy stub user
            ...
            if (userCheckAfter.rows.length === 0) {
              return reply.status(404).send({
                error: "User not found in portal or local directory.",
              });
            }
          }
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        if (ownerRes.rows[0].user_id !== user.id) {
          return reply.status(403).send({
            error: "Forbidden: Only the owner can edit this dashboard",
          });
        }
  ...
  fastify.get(
    ...
    async (request: any, reply) => {
      ...
      if (
        !section ||
        !["key_skill", "gap", "training_plan"].includes(section)
      ) {
        return reply.status(400).send({
          error:
            'Valid section ("key_skill", "gap", "training_plan") is required.',
        });
      }
      try {
        const res = await db.execute({
  ...
  fastify.post(
    ...
    async (request: any, reply) => {
      ...
      if (!section || !title) {
        return reply
          .status(400)
          .send({ error: "Section and Title are required." });
      }
      ...
      try {
        ...
        if (ownerRes.rows[0].user_id !== user.id) {
          return reply.status(403).send({
            error: "Forbidden: Only the owner can add items to this dashboard",
          });
        }
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        if (res.rows[0].user_id !== user.id) {
          return reply
            .status(403)
            .send({ error: "Forbidden: Only the owner can delete this item" });
        }
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        if (res.rows[0].user_id !== user.id) {
          return reply
            .status(403)
            .send({ error: "Forbidden: Only the owner can update this item" });
        }
        ...
        const newTargetQuarter =
          target_quarter !== undefined
            ? target_quarter
            : current.target_quarter;
        const newCompletedQuarter =
          completed_quarter !== undefined
            ? completed_quarter
            : current.completed_quarter;
  ...
  fastify.post(
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        const pName = program_line?.trim() || "New Program";
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        if (original.user_id !== user.id) {
          return reply.status(403).send({
            error: "Forbidden: Only the owner can duplicate this dashboard",
          });
        }
          ...
          for (const item of itemsRes.rows) {
            ...
            await tx.execute({
              ...
              args: [
                ...
                item.status,
                item.target_quarter,
                item.completed_quarter,
              ],
            });
          }

          // Clone links mapping old item IDs to new item IDs
          const linksRes = await tx.execute({
            sql: "SELECT * FROM dashboard_item_links WHERE dashboard_id = ?",
            args: [dashboardId],
          });
  ...
  fastify.post(
    ...
    async (request: any, reply) => {
      ...
      if (!source_id || !Array.isArray(target_ids)) {
        return reply
          .status(400)
          .send({ error: "source_id and target_ids array are required." });
      }
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        if (dashRes.rows[0].user_id !== user.id) {
          return reply.status(403).send({
            error: "Forbidden: Only the owner can delete this dashboard",
          });
        }
        ...
        if (count <= 1) {
          return reply.status(400).send({
            error:
              "Cannot delete your only dashboard. You must have at least one program.",
          });
        }
  ...
  fastify.post(
    "/api/dashboard/:id/versions/:versionId/restore",
    { preValidation: [fastify.authenticate] },
    async (request: any, reply) => {
      const user = request.user;
      const dashboardId = request.params.id;
      ...
      try {
        ...
        try {
          ...
          await tx.execute({
            ...
            args: [
              snapshot.program_line || "Default Program",
              snapshot.objective || "",
              snapshot.notes || "",
              dashboardId,
            ],
  ...
  fastify.delete(
    "/api/dashboard/:id/versions/:versionId",
    { preValidation: [fastify.authenticate] },
    async (request: any, reply) => {
      const user = request.user;
      const dashboardId = request.params.id;
