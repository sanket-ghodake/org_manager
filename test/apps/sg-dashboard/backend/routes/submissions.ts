...
async function autoSubmitExpiredRequests() {
  ...
  // Use native transaction to ensure atomic execution
  ...
  try {
    ...
    for (const req of expired.rows) {
      const dashRes = await tx.execute({
      ...
      console.log(
        `[AUTO-SUBMIT] Request ${req.id} for employee ${req.employee_id} was automatically submitted because the deadline ${req.deadline} has passed.`,
      );
    }
    await tx.commit();
  } catch (err: any) {
    ...
    async (request: any, reply) => {
      ...
      if (!isAuthorized) {
        return reply.status(403).send({
          error: "Forbidden: You are not authorized to view these submissions.",
        });
      }
  ...
  fastify.post(
    ...
    async (request: any, reply) => {
      ...
      if (!employee_id || !deadline) {
        return reply
          .status(400)
          .send({ error: "employee_id and deadline are required." });
      }
      ...
      try {
        ...
        if (
          immediateManagerId !== user.id &&
          !isUpline &&
          user.role !== "Admin"
        ) {
          return reply.status(403).send({
            error:
              "Forbidden: You can only request submissions from your reporting line.",
          });
        }
        ...
        console.log(
          `\x1b[35m[ALERT] Real-time email alert sent to ${employeeName} (${employee_id}): Manager ${user.name} has requested a new SG Dashboard plan submission with deadline ${deadline}.\x1b[0m`,
        );
      ...
      try {
        ...
        if (!managerId) {
          return reply.status(400).send({
            error:
              "Submission failed: You do not have a manager assigned to you in the system. Please contact your administrator.",
          });
        }
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        if (reqRes.rows.length === 0) {
          return reply
            .status(404)
            .send({ error: "Submission request not found." });
        }

        const row = reqRes.rows[0] as any;
        if (row.employee_id !== user.id) {
          return reply.status(403).send({
            error: "Forbidden: Only the assigned employee can submit.",
          });
        }
        ...
        if (row.deadline < nowStr) {
          return reply.status(400).send({
            error:
              "Submission deadline has passed and this request is frozen. Your last active dashboard version has been locked for review.",
          });
        }
        ...
        if (row.status !== "Pending" && row.status !== "Needs Revision") {
          return reply
            .status(400)
            .send({ error: "This request is already submitted or approved." });
        }
        ...
        console.log(
          `\x1b[35m[ALERT] Real-time chat notification sent to ${managerName}: Employee ${user.name} has submitted their SG Dashboard plan for review.\x1b[0m`,
        );
  ...
  fastify.post(
    ...
    async (request: any, reply) => {
      ...
      if (!status || !["Approved", "Needs Revision"].includes(status)) {
        return reply.status(400).send({
          error: 'Valid status ("Approved" or "Needs Revision") is required.',
        });
      }
      ...
      try {
        ...
        if (row.manager_id !== user.id && user.role !== "Admin") {
          return reply.status(403).send({
            error:
              "Forbidden: You can only review submissions assigned to you.",
          });
        }
        ...
        const _employeeName = empCheck.rows[0]?.name || "Employee";
        console.log(
          `\x1b[35m[ALERT] Real-time notification sent to dots: Your SG Dashboard plan has been reviewed by ${user.name}. Status: ${status}. Feedback: "${feedback || "None"}".\x1b[0m`,
        );
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        if (row.manager_id !== user.id && user.role !== "Admin") {
          return reply.status(403).send({
            error:
              "Forbidden: You can only freeze submissions assigned to you.",
          });
        }
        ...
        if (row.status !== "Pending") {
          return reply
            .status(400)
            .send({ error: "Only pending submission requests can be frozen." });
        }
