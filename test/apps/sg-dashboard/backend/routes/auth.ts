  ...
  fastify.post("/api/auth", async (request: any, reply) => {
    ...
    try {
      ...
      if (!exchangeRes.ok) {
        ...
        throw new Error(
          `Token exchange failed (${exchangeRes.status}): ${errText}`,
        );
      }
      ...
      if (mappedRole === "Employee") {
        try {
          const checkRes = await fetch(
            `${PORTAL_INTERNAL_URL}/api/directory?managerId=${user.id}`,
            {
              headers: {
          ...
          console.warn(
            "Failed to dynamically check manager status on login:",
            err.message,
          );
        }
      }
      ...
      return reply
        .status(401)
        .send({ error: `Authorization failed: ${err.message}` });
    }
  });
  ...
  fastify.post(
    ...
    async (request: any, reply) => {
      ...
      if (!Array.isArray(users)) {
        return reply
          .status(400)
          .send({ error: "Invalid user directory payload" });
      }

      if (user.role !== "Admin") {
        // Enforce self-sync or manager-subordinate sync authorization
        for (const targetUser of users) {
          if (!targetUser || typeof targetUser.id !== "string") {
            return reply
              .status(400)
              .send({ error: "Invalid sync request: missing target user id." });
          }

          const isAuthorized =
            user.id === targetUser.id ||
            (await checkUplineManager(user.id, targetUser.id));

          if (!isAuthorized) {
            return reply.status(403).send({
              error: `Forbidden: You are not authorized to sync user profile: ${targetUser.id}.`,
            });
          }
        }
      }
      ...
      try {
        ...
        try {
          ...
          console.warn(
            "Failed to fetch portal directory for sync validation:",
            err.message,
          );
        }
        ...
        for (const u of users) {
          ...
          const portalRole = verifiedUser
            ? (verifiedUser.role || "").toLowerCase()
            : "";
          ...
          if (managerId) {
            ...
            if (circularCheck.rows && circularCheck.rows.length > 0) {
              console.warn(
                `Circular reporting loop detected for user ${id} reporting to ${managerId}. Skipping manager association.`,
              );
              continue;
            }
          }
  ...
  fastify.get(
    ...
    async (request: any, reply) => {
      ...
      try {
            ...
            Promise.resolve().then(async () => {
              for (const u of users) {
                ...
                try {
                  ...
                  console.error(
                    "Background sync user write error:",
                    dbErr.message,
                  );
                }
              }
        ...
        console.error(
          "All portal directory proxy requests failed, falling back to local database lookup:",
          err.message,
        );
      }
      ...
      try {
        ...
        return reply.status(502).send({
          error: "Failed to retrieve directory from portal or local database",
        });
      }
    },
  );
