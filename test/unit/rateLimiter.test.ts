...
describe("In-Memory Rate Limiter", () => {
  test("Allows requests under the limit", () => {
    ...
    for (let i = 0; i < 5; i++) {
      const res = isRateLimited(ip, namespace, 5, 10000);
  ...
  test("Blocks requests exceeding the limit", () => {
    ...
    const blockedRes = isRateLimited(ip, namespace, 5, 10000);
    expect(blockedRes.limited).toBe(true);
    expect(blockedRes.current).toBe(5);
  });

  test("Is namespace isolated", () => {
    const ip = "9.10.11.12";
    
    // Fill up namespace A
    for (let i = 0; i < 5; i++) {
      isRateLimited(ip, "namespace-a", 5, 10000);
    }
    expect(isRateLimited(ip, "namespace-a", 5, 10000).limited).toBe(true);

    // Namespace B should still allow requests for the same IP
    const resB = isRateLimited(ip, "namespace-b", 5, 10000);
